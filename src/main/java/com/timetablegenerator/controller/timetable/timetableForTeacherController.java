...
public class timetableForTeacherController {
    @FXML
    ...
    public void loadTimetableData() throws SQLException {

        lblHeaderInfo.setText("Weekly Timetable of " + authSession.getUser().getName());

        timetableGrid.getChildren().removeIf(node -> node instanceof VBox);

        List<TimetableDetailDTO> details = service.getTimetableListTeacher();

        for (TimetableDetailDTO slot : details) {
            if (slot.getTeacher_id() == null && slot.getCourseName() == null) {
                VBox cell = createBlankCell(slot);
    ...
    private VBox createCell(TimetableDetailDTO data) {
        ...
        Label teacher = new Label(data.getClass_name());
    ...
    private VBox createBlankCell(TimetableDetailDTO data) {
        VBox box = new VBox(0);
        box.setAlignment(Pos.CENTER);
        box.getStyleClass().add("timetable-cell");
        box.setMinHeight(80);
