...
public class availabilityListTeacherController {
    @FXML
    ...
    private void loadTableData(int pageIndex) {
        try {
            ...
            List<availabilityModel> list = service.getAvailabilityByTeacherId(pageIndex + 1, rowsPerPage, searchText);
    ...
    private void updatePaginationCount() {
        try {
            String searchText = (searchField.getText() == null) ? "" : searchField.getText().trim();
            int totalRecords = service.getTotalAvailabilityCountByTeacher(searchText);
        ...
        colActions.setCellFactory(param -> new TableCell<>() {
...

            {
                ...
                editItem.setOnAction(event -> {
                    availabilityModel model = getTableView().getItems().get(getIndex());
                    handleEdit(model.getId());
                });

                deleteItem.setOnAction(event -> {
                    availabilityModel model = getTableView().getItems().get(getIndex());
                    handleDelete(model.getId());
                });
            }
    ...
    private void openCreateForm() {
        showForm("/view/availability/availabilityCreate.fxml", "Create Availability", 0);
    }
    ...
    private void handleEdit(int id) {
        showForm("/view/availability/availabilityEdit.fxml", "Edit Availability", id);
    }
    ...
    private void showForm(String fxmlPath, String title, int id) {
        try {
            ...
            } else if (controller instanceof availabilityEditController && id > 0) {
                ((availabilityEditController) controller).loadAvailabilityData(id);
            }
            ...
            System.err.println("Error loading form: " + fxmlPath);
            e.printStackTrace();
        }
    }
